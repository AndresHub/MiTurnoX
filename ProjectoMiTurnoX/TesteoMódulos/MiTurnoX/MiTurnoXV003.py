...
def definir_atenciones(num):
    for i in range(num):
        ...
        texto_atencion = StringVar(frame_submenu_configuracion_atenciones)
        ...
        texto_letra = StringVar(frame_submenu_configuracion_atenciones)
    ...
    def crear_atenciones():
        print(f"{letra_atenciones[0].get()}, {letra_atenciones[1].get()} ")
        for i in range(0, num):
            boton_atencion_definida = ctk.CTkButton(
                frame_atenciones,
                text= (f"{texto_botones[i].get()}"),
                command= lambda i=i: crear_turno_atencion(f"{letra_atenciones[i].get()}")
            )
    ...
    def crear_turno_atencion(letra):
        print(letra)
